...
RUN curl -s -L "${MAVEN_DOWNLOAD_URL}" > "/tmp/apache-maven-3.5.4-bin.zip" \
&& unzip -qq "/tmp/apache-maven-3.5.4-bin.zip" -d "/data" \
&& mv "/data/apache-maven-3.5.4" "/data/apache-maven" \
&& rm -f "/tmp/apache-maven-3.5.4-bin.zip"
